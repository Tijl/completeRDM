%% We're using real RDMs here, test different completion methods (geometric, graph & MDS)
clearvars; close all; clc

% Set the random seed (so we get the same random RDMs each time)
rng(1);
num_iter = 100;             %  Number of iterations (i.e. random cells masked)
perc_missing = 1:80;        % percentage of missing values we want to test
num_methods = 3;            % geometric, graph & MDS
...
corr_orig_full = zeros(num_iter,numel(perc_missing),num_datasets,num_methods);
corr_orig_reconstructed = zeros(num_iter,numel(perc_missing),num_datasets,num_methods);
...
for dataset_idx = 1:num_datasets
    ...
    for iter = 1:num_iter
        ...
        for perc_missing_idx = 1:numel(perc_missing) % percentage of missing values
            ...
            % Loop over the different methods (geometric, graph & MDS)
            for method = 1:num_methods
                ...
                switch method
                    ...
                    case 2 % Graph
                        Y = complete_rdm_graph(X);
                    case 3 % MDS
                        Y = complete_rdm_mds(X,2);
                end

                % Check the correlation between the full original and reconstructed RDMs
                corr_orig_full(iter,perc_missing_idx,dataset_idx,method) = corr(squareform(Y)',Xt',"type","Pearson");
                ...
                corr_orig_reconstructed(iter,perc_missing_idx,dataset_idx,method) = corr(X_2d(nan_idx),Y_2d(nan_idx),"type","Pearson");
...
% Save 
save('analysis_3.mat','corr_orig_full','corr_orig_reconstructed');
